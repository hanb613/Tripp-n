<template>
  <b-container class="bv-example-row mt-3 ml-1">
    <b-row>
      <b-col>
        <b-alert variant="secondary" show><h3 style="font-family: 'Noto Sans KR', sans-serif">비밀번호 찾기</h3></b-alert>
      </b-col>
    </b-row>
    <b-row>
      <b-col></b-col>
        ...
        <b-card class="text-center mt-3" style="max-width: 40rem" align="left">
          <b-form class="text-left">
            ...
            <b-alert
              show
              variant="primary"
              v-if="!isPwFindError && userFindPw !== ''"
              >회원님의 PW :
              <span style="text-decoration: underline"
                ><b>{{ this.userFindPw }}</b></span
              ></b-alert
            >
            <br />
    ...
    async confirm() {
      ...
      if (this.isPwFindError) {
        ...
        if (this.userFindPw.length <= 2) {
          this.$store.commit(
            `${memberStore}/SET_USER_FIND_PW`,
            "*".repeat(this.userFindPw.length)
          );
        } else {
          this.$store.commit(
            `${memberStore}/SET_USER_FIND_PW`,
            this.userFindPw.slice(0, 2) + "*".repeat(this.userFindPw.length - 2)
          );
        }
      }
    },
